...
import sqlite3
from config1 import PG_HOST, PG_PORT, PG_USER, PG_PASSWORD, PG_DATABASE
...
async def start_handler(message: types.Message, state: FSMContext):
    # print(f'user/start_handler/{message.from_user.id}')
    user_id = message.from_user.id
    with sqlite3.connect('users.db') as conn:
        # print(f'user/start_handler/sqlite3.connect/{conn.cursor()}')
        cursor = conn.cursor()
        cursor.execute("SELECT * FROM users WHERE telegram_user_id = ?", (user_id,))
        # print(f'user/start_handler/with/{cursor.fetchone()}')
        if cursor.fetchone():
